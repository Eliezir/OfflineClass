...
/**
 * Wire up GitHub-based auto-update for the student app.
 *
 * Offline-first: the check fires once shortly after launch and every failure
 * (no network) is swallowed. When an update is downloaded the user is asked
 * whether to restart now or later. The student build publishes under the
 * `student` channel, so its `app-update.yml` points at `student.yml` in the
 * shared GitHub Release — nothing to configure here.
 */
export function initAutoUpdater(getWindow: () => BrowserWindow | null): void {
  ...
  autoUpdater.on('update-downloaded', (info) => {
    const options = {
      ...
      message: `A versão ${info.version} do OfflineClass — Aluno foi baixada.`,
  ...
  // Being offline is expected — log, never interrupt the student.
